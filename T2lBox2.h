...
#ifdef LESS11
	Box2() : Box<T, 2>() 
	{ Tuple<Interval<T>, 2>::set(0, Interval<T>()); Tuple<Interval<T>, 2>::set(1, Interval<T>()); }
	Box2( const Interval<T>& x, const Interval<T>& y ) : Box<T, 2>() 
	{ Tuple<Interval<T>, N>::set(0, x); Tuple<Interval<T>, N>::set(1, y); }
#else
